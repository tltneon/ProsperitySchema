...
function SWEP:Think()	
	...
	if SERVER then
		if ( self.Owner:KeyDown( IN_ATTACK ) && self.Owner:isWepRaised() && !self:GetDTBool(3) ) then
			local character = self.Owner:getChar()
			if self.timeCooldown < CurTime() then
				local stam = self.Owner:getLocalVar("stm", 0)
				local redc = ( character:getAttrib("strength", 0) / 100 )
...
function SWEP:FireBullet( spr, ang, hit )
		...
		if SERVER then
			...
			dam = dam * ( 1 + character:getAttrib("end", 0)/100 )
			dam = dam / 3
		end
...
function SWEP:DoDamage()
	...
	local character = self.Owner:getChar()
	local stam = self.Owner:getLocalVar("stm", 0)
	local redc = ( character:getAttrib("end", 0) / 100 )
	--self.Owner:setLocalVar("stm", (stam - ( self.Primary.Stamina - redc * self.Primary.Stamina )))
	for i = -1, 1 do
