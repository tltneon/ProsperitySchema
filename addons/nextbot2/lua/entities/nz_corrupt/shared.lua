...
function ENT:CustomDeath( dmginfo )
	...
	if dmginfo:IsExplosionDamage() or self.Healing then
		self:BecomeRagdoll(dmginfo)
	else
		self:DeathAnimation( "nz_deathanim_base", self:GetPos(), ACT_HL2MP_WALK_ZOMBIE_06, 1 )
	end
...
ENT.nxtHeal = 0
function ENT:Heal( amount, time, dmginfo )

	if !self.nxtHeal then self.nxtHeal = 0 end
    if CurTime() < self.nxtHeal then return end

    self.nxtHeal = CurTime() + 10

	dmginfo:SubtractDamage(50)
	self.Healing = true
...
function ENT:CustomChaseEnemy()
	if self.Healing then
	self.loco:FaceTowards(self:GetPos())
	end
end
