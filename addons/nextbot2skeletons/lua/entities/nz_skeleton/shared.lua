...
function ENT:Initialize()
	...
	if anim == 1 then self.WalkAnim = ACT_HL2MP_RUN_ZOMBIE_FAST
	elseif anim == 2 then self.WalkAnim = ACT_HL2MP_RUN_ZOMBIE
	elseif anim == 3 then self.WalkAnim = ACT_HL2MP_RUN_FAST
	end
...
function ENT:Leap()
	...
	if !self.nxtLeap then self.nxtLeap = 0 end
    if CurTime() < self.nxtLeap then return end

    self.nxtLeap = CurTime() + 1

	local enemy = self:GetEnemy()
	local JumpHight = math.Clamp ( self:GetRangeTo( enemy ) ,100 ,300 ) 
	local RumpAcSpeed = math.Clamp ( self:GetRangeTo( enemy ) ,0 ,500 ) 
	
	self.loco:SetJumpHeight( JumpHight / 2 )
	self.loco:Jump( self.Enemy:GetPos() * 15 )
	self.loco:SetAcceleration( RumpAcSpeed + 200 * 5)
	self.loco:SetDesiredSpeed( self.Speed * 3 )
	self:StartActivity( ACT_HL2MP_JUMP_KNIFE )
	
	self:EmitSound("npc/fast_zombie/leap1.wav", 70, math.random(70,85) )
