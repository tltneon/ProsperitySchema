...
function ENT:Initialize()
	...
	--Stats--
	if GetConVarNumber("nightmare_collide") == 0 then
	self:SetCollisionGroup(COLLISION_GROUP_DEBRIS)
	end
	
	self:SetModel(self.Model)
	self:SetHealth(GetConVarNumber("nightmare_health"))
	self:SetModelScale( 1.1, 0 )
	
	self.LoseTargetDist	= GetConVarNumber("nightmare_distcheck")	-- How far the enemy has to be before we lose them
	self.SearchRadius 	= GetConVarNumber("nightmare_dist")	-- How far to search for enemies
...
function ENT:AttackProp()
	...
	for _,v in pairs(entstoattack) do
		...
		if (self:GetRangeTo(v) < 65) then
			...
			v:TakeDamage(GetConVarNumber("nightmare_damage"), self)	
...
if SERVER then
		...
		self.Enemy:TakeDamage(GetConVarNumber("nightmare_damage"), self)	
		...
		-- Slow
		if GetConVarNumber("nightmare_slow") == 1 then
		self.Enemy:SetRunSpeed(GetConVarNumber("nightmare_amount"))
		self.Enemy:SetWalkSpeed(GetConVarNumber("nightmare_amount"))
