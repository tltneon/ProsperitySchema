...
function ENT:Initialize()
	...
	if GetConVarNumber("ticklemonster_collide") == 0 then
	self:SetCollisionGroup(COLLISION_GROUP_DEBRIS)
	end
	
	self:SetModel(self.Model)
	self:SetHealth(GetConVarNumber("ticklemonster_health"))
	self:SetModelScale( 1.2, 0 )
	
	self.LoseTargetDist	= GetConVarNumber("ticklemonster_distcheck")	-- How far the enemy has to be before we lose them
	self.SearchRadius 	= GetConVarNumber("ticklemonster_dist")	-- How far to search for enemies
	...
	local vecSpineOffset = Vector(6, 0, 0) 
	for _, bone in pairs(bones) do
		local spineid = self:LookupBone(bone)
		if spineid and spineid > 0 then
			self:ManipulateBonePosition(spineid, vecSpineOffset)
		end
	
	end
	
	local vecSpineOffset = Vector(8, 0, 0) -- scale the spine
local SpineBones = {"ValveBiped.Bip01_Spine2", "ValveBiped.Bip01_Spine4", "ValveBiped.Bip01_Spine3"} -- find the spine
...
function ENT:AttackProp()
	...
	for _,v in pairs(entstoattack) do
		...
		return end
			...
			if (self:GetRangeTo(v) < 115) then
			
		v:EmitSound(self.Hit)
		v:TakeDamage(GetConVarNumber("ticklemonster_damage"), self)	
		...
		self:PlaySequenceAndWait( "zombie_attack_06_original", 1.4 )
...
function ENT:RunBehaviour()
	while ( true ) do
		...
		self.HasNoEnemy = false
			...
			self.loco:SetDesiredSpeed(GetConVarNumber("ticklemonster_speed"))
			self:ChaseEnemy() 
		else
			-- Wander around
...
if SERVER then
		...
		return end
			...
			if (self:GetRangeTo(self.Enemy) < 115) then
			
		self:EmitSound(self.Hit)
		self.Enemy:TakeDamage(GetConVarNumber("ticklemonster_damage"), self)	
