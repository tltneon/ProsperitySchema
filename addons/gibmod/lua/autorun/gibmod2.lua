if CLIENT then
	...
	local effectTime = CreateConVar( "gibmod_effecttime", "120", { FCVAR_ARCHIVE, FCVAR_DEMO, FCVAR_REPLICATED } )
	...
	function GibMod_CSEffect( len )
		...
		if effect_type == 1 then
			local ef = EffectData()
				...
				ef:SetScale( 20 )
			util.Effect( "dismemberment_bloodgibs", ef )
...
local bodyGibs = {	-- body gibs for full body explosion
					"models/dismemberment/gibs/arms.mdl",
					"models/dismemberment/gibs/legs.mdl",
					"models/gibs/hgibs.mdl",
					"models/gibs/antlion_gib_small_3.mdl",
					"models/dismemberment/gibs/headgibs.mdl",
					"models/dismemberment/gibs/arms/hand.mdl",
					"models/dismemberment/gibs/arms/lower_arm.mdl",
					"models/dismemberment/gibs/arms/upper_arm.mdl",
					"models/dismemberment/gibs/legs/foot.mdl",
...
local centralExplodeForce = 10000 -- force required to explode a ragdoll when hit in a central bone
local explodeForce = 6000 -- force required to explode a ragdoll
local explosionDamage = 100 -- damage required for an explosion to explode a ragdoll
local limbDamage = 100 -- damage required to dismember a limb
...
local minGibs = 4 -- min number of body gibs
local maxGibs = 6 -- max number of body gibs
local numHgibs = 1 -- number of headshot gibs

local numBloodStreams = 10 -- number of blood streams
...
local ragdollTime = CreateConVar( "gibmod_ragdolltime", "999999999", { FCVAR_ARCHIVE, FCVAR_DEMO, FCVAR_REPLICATED } )
...
function GibMod_Explode( ent, damageForce, isExplosionDamage )
	...
	for i = 1, numStringExplosions do		
		...
		for i = 1, numBloodStreams / numStringExplosions do
			...
			local constraint, rope = constraint.Rope( droplet, origin, 0, 0, Vector(0, 0, 0), Vector(0, 0, 0), len, 0, 5000, 15, "gibmod/bloodstream", false )
			droplet.rope = rope
...
function GibMod_Dismember( ent, damagePos, damageForce, isExplosionDamage )
	...
	if ( string.find( string.lower( ent:GetBoneName( hitBoneObject ) ), "head" ) ) then
		-- sound
		sound.Play( "dismemberment/body_head_01.wav", damagePos )
