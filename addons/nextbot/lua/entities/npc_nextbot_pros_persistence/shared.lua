...
ENT.AttackWaitTime = .75
ENT.AttackFinishTime = .75
...
function ENT:ChaseEnemy( options )
	...
	local ent = ents.FindInSphere( self:GetPos(), 65) 
	...
	if v:Health() > 40 then			
		...
		v:SetVelocity(moveAdd+((self.Enemy:GetPos()-self:GetPos()):GetNormal()*250)) -- apply the velocity
		
		else
		self:RestartGesture(ACT_GMOD_GESTURE_TAUNT_ZOMBIE)
		
		self:EmitSound(self.Enrage)
		
		v:EmitSound("vo/npc/male01/no02.wav")
		
		v:DoAnimationEvent(ACT_GMOD_GESTURE_TAUNT_ZOMBIE) 
		
		v:SetRunSpeed(0.1)
		v:SetWalkSpeed(0.1)
		
		coroutine.wait(2) -- wait for taunt to finish	
		
		self:RestartGesture(ACT_GMOD_GESTURE_MELEE_SHOVE_1HAND) -- punch
		
		coroutine.wait(self.AttackFinishTime) -- wait for attack to finish
		
		v:ViewPunch(Angle(math.random(-1, 1)*self.Damage, math.random(-1, 1)*self.Damage, math.random(-1, 5)*self.Damage))
		
		v:EmitSound(self.Hit)
		
		v:EmitSound("physics/flesh/flesh_bloody_break.wav")
		
		v:TakeDamage(math.huge, self)	
