...
function ENT:Initialize()
	...
	local randommodel = math.random(1,12)
	...
	if randommodel == 6 then
	self:SetModel(self.Model6)
	self.WalkAnim = ACT_HL2MP_RUN_FAST
	end
	if randommodel == 7 then
	self:SetModel(self.Model7)
	self.WalkAnim = ACT_HL2MP_RUN_CHARGING
	end
	if randommodel == 8 then
	self:SetModel(self.Model8)
	self.WalkAnim = ACT_HL2MP_RUN_ZOMBIE
	end
	if randommodel == 9 then
	self:SetModel(self.Model9)
	self.WalkAnim = ACT_HL2MP_RUN_CHARGING
	end
	if randommodel == 10 then
	self:SetModel(self.Model10)
	self.WalkAnim = ACT_HL2MP_RUN_FAST
	end
	if randommodel == 11 then
	self:SetModel(self.Model11)
	self.WalkAnim = ACT_HL2MP_RUN_FAST
	end
	if randommodel == 12 then
	self:SetModel(self.Model12)
	self.WalkAnim = ACT_HL2MP_RUN_ZOMBIE
	end
	end
...
function ENT:AttackProp()
	...
	for _,v in pairs(entstoattack) do
		...
		self:EmitSound("npc/infected_zombies/rage_at_victim"..math.random(20,31)..".wav", math.random(75,95), math.random(65,105))
	
		self:RestartGesture(self.AttackAnim)  
		self:SetPoseParameter("move_x",0)
		
		if !self.WalkAnim == ACT_RUN then
...
function ENT:IdleSounds()
	if SERVER then
	self:EmitSound("npc/infected_zombies/been_shot_"..math.random(16)..".wav", math.random(75,95), math.random(80,105))
	end
end

function ENT:AlertSound()
	if SERVER then
	self:EmitSound("npc/infected_zombies/rage_at_victim"..math.random(32,37)..".wav", math.random(75,95), math.random(80,105))
	end
end
...
function ENT:Attack()
    ...
    self.nxtAttack = CurTime() + 1.2
		...
		if not (self.WalkAnim == ACT_RUN) then
		...
		if self.Enemy:IsPlayer() then
			self.Enemy:EmitSound("npc/infected_zombies/hit_punch_0"..math.random(8)..".wav", math.random(100,125), math.random(85,105))
			self.Enemy:TakeDamage( self.Damage -5, self)	
			self.Enemy:ViewPunch(Angle(math.random(-1, 1)*self.Damage, math.random(-1, 1)*self.Damage, math.random(-1, 1)*self.Damage))
...
function ENT:OnKilled( dmginfo )
	self:EmitSound("npc/infected_zombies/death_"..math.random(17,35)..".wav", math.random(75,95), math.random(65,95))
	
	local zombie = ents.Create("nextbot_zombies_deathanim")
			if !self:IsValid() then return end
...
function ENT:OnInjured( dmginfo )
	...
	if math.random(1,3) == 1 then
		...
		self:EmitSound("npc/infected_zombies/pain"..math.random(9)..".wav", math.random(95,95), math.random(65,95))
		end
	end
