...
function ENT:Initialize()
	...
	self.LoseTargetDist	= 999999999999	-- How far the enemy has to be before we lose them
	...
	model2:SetModelScale(1, 0)
    model2:Spawn()
    model2:Fire("setparentattachment", "Anim_Attachment_RH", 0.01)
	model2:SetCollisionGroup(COLLISION_GROUP_DEBRIS)
	model2:SetHealth(9999999)
	end
...
function ENT:ChaseEnemy( options )
	...
	local ent = ents.FindInSphere( self:GetPos(), 70) 
...
function ENT:OnInjured( dmginfo )
	...
	if self:Health() < 1000 then -- if my health is above 1000 then take 30% reduced dmg else take 50% reduced dmg
