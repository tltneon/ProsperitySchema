	...
	local door = ents.FindInSphere(self:GetPos(),25)
		...
		return end
							self:EmitSound(self.DoorBreak)
								...
								self:PlaySequenceAndWait( "zombie_attack_special_original", 1.4 )
		...
		local ent = ents.FindInSphere(self:GetPos(),150)
...
function ENT:Attack()
	...
	sounds[2] = (self.Attack2)
		...
		if (self:GetRangeTo(self.Enemy) < 90) then
...
function ENT:OnKilled( dmginfo )
	...
	local zombie = ents.Create("no_ragdoll_deathanim")
			if !self:IsValid() then return end
			...
			zombie:Spawn()
			zombie:StartActivity(ACT_HL2MP_RUN_ZOMBIE)
